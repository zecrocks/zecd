...
//! That is the system-level proof that the two backends derive the same chain view (a byte-order
...
//! zebra-backed like everywhere else, which is what makes the outage phases below safe: pausing or
...
async fn regtest_lwd_e2e() {
    let (Some(zebrad_bin), Some(lwd_bin)) =
        ...
        eprintln!(
            "SKIP regtest_lwd_e2e: set ZEBRAD_BIN and LIGHTWALLETD_BIN to run the light-mode \
             e2e. The harness still compiled and linked."
        );
        return;
    };

    // 1-2. One chain and its funding wallet, exactly like the funded e2e. The funder is
    //      zebra-backed, so - unlike the old devtool funder, which needed a second lightwalletd
    //      purely for fault isolation - the outage phases below cannot reach it.
    ...
    funder
        .send_with_memo(&zecd_ua, FUND_ZATOSHIS, Some(memo))
        .await
