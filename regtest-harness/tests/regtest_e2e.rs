...
//!
//! The **unfunded** surface, and deliberately only that: a daemon comes up against a real chain,
//! follows it, and answers correctly with an empty wallet. Every funded flow belongs to
//! `regtest_funded.rs` and its siblings, so nothing that needs a funder should be added here -
//! this binary is cheap (one zebrad, one zecd, no funder, no proving) and that is its value.
//!
...
async fn regtest_end_to_end() {
    ...
    // The data-directory layout migration used to be re-checked here on this fundless wallet.
    // It is covered by `regtest_migration.rs` instead, and only there: on a wallet with no funds
    // a migration that silently did not happen is indistinguishable from one that did - the
    // daemon just rebuilds an empty database from `keys.toml` and rescans, and every assertion
    // this phase made (databases in the new place, `keys.toml` in the old one, `ismine` on an
    // address the account can always re-derive) still passes. Only funds, history, a memo and
    // the block cache show the data was *moved*, which is what that binary asserts.
}
