...
// =============================== tparty (auto-shielding deposit daemon) ===============================

/// Locate the built `tparty` binary: `$TPARTY_BIN` if set, else the parent crate's release build.
pub fn tparty_bin() -> PathBuf {
    if let Ok(p) = std::env::var("TPARTY_BIN") {
        return PathBuf::from(p);
    }
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest
        .parent()
        .map(|p| p.join("target/release/tparty"))
        .unwrap_or_else(|| PathBuf::from("tparty"))
}

/// How `tparty` should reach the regtest lightwalletd, plus its auto-shield policy.
pub struct TpartyConfig {
    pub lightwalletd_port: u16,
    pub rpc_port: u16,
    pub rpc_user: String,
    pub rpc_password: String,
    /// `[tparty] min_conf` - confirmations a deposit needs before auto-shielding.
    pub min_conf: u32,
    /// `[tparty] threshold_zat` - don't shield below this total.
    pub threshold_zat: u64,
}

impl TpartyConfig {
    /// Test-friendly defaults: shield at 1 confirmation, with a threshold (0.0015 ZEC) low
    /// enough that the main test deposits shield promptly but high enough that a dedicated
    /// sub-threshold deposit can exercise the skip + `shieldfunds` path.
    pub fn new(lightwalletd_port: u16, rpc_port: u16) -> TpartyConfig {
        TpartyConfig {
            lightwalletd_port,
            rpc_port,
            rpc_user: "user".to_string(),
            rpc_password: "pass".to_string(),
            min_conf: 1,
            threshold_zat: 150_000,
        }
    }
}

/// A running `tparty` daemon plus the HTTP client and credentials to drive it. Mirrors
/// [`Zecd`] (same JSON-RPC wire format and harness contract).
pub struct Tparty {
    child: Child,
    base_url: String,
    user: String,
...
impl Tparty {
    /// Write a regtest `tparty.toml`, run `tparty init` (retried while lightwalletd catches
    /// up), then spawn the daemon. Returns once the RPC is up.
    pub async fn start(cfg: &TpartyConfig) -> Result<Tparty> {
        let datadir = tempfile::tempdir().context("create tparty datadir")?;
        let bin = tparty_bin();
        if !bin.exists() {
            bail!(
                "tparty binary not found at {} - build it first (cargo build --release --bin \
                 tparty) or set $TPARTY_BIN",
                bin.display()
            );
        }

        write_tparty_toml(datadir.path(), cfg).context("write tparty.toml")?;

        // `tparty init` contacts lightwalletd (get_latest_block + get_tree_state). Just after
        // launch lightwalletd may still be ingesting from zebrad, so retry, resetting the
        // datadir between attempts so a partial init can't wedge the next one.
        let deadline = Instant::now() + Duration::from_secs(90);
        loop {
            let init = Command::new(&bin)
                ...
                .stderr(Stdio::piped())
                .output()
                .context("spawn tparty init")?;
            if init.status.success() {
                break;
            }
            if Instant::now() >= deadline {
                bail!(
                    "tparty init failed after retries ({}):\n{}",
                    init.status,
                    String::from_utf8_lossy(&init.stderr)
                );
            }
            reset_tparty_datadir(datadir.path(), cfg)?;
            tokio::time::sleep(Duration::from_secs(2)).await;
        }
        ...
        tparty.wait_until_rpc_up().await?;
        Ok(tparty)
    }

    /// Issue a JSON-RPC call, returning the `result` on success or an error carrying the
    /// Bitcoin Core error `code`.
    pub async fn call(&self, method: &str, params: Value) -> Result<Value, RpcError> {
        let body = json!({ "jsonrpc": "1.0", "id": "harness", "method": method, "params": params });
        let resp = self
            .http
    ...
    /// Poll until `getblockchaininfo.blocks` reaches `target`.
    pub async fn wait_until_synced(&self, target: u64, timeout: Duration) -> Result<()> {
        let deadline = Instant::now() + timeout;
        loop {
            if let Ok(info) = self.call("getblockchaininfo", json!([])).await {
            ...
            if Instant::now() >= deadline {
                bail!("tparty did not sync to height {target} within {timeout:?}");
            }
            tokio::time::sleep(Duration::from_millis(500)).await;
        }
    }
    ...
    async fn wait_until_rpc_up(&self) -> Result<()> {
        ...
        loop {
            ...
            if Instant::now() >= deadline {
                bail!("tparty RPC did not come up within 30s");
            }
            tokio::time::sleep(Duration::from_millis(300)).await;
        }
    }
...
fn reset_tparty_datadir(datadir: &Path, cfg: &TpartyConfig) -> Result<()> {
    ...
    write_tparty_toml(datadir, cfg)
}

fn write_tparty_toml(datadir: &Path, cfg: &TpartyConfig) -> Result<()> {
    let toml = format!(
        r#"network = "regtest"
datadir = "{datadir}"
...
"#,
        ...
        lwd_port = cfg.lightwalletd_port,
        rpc_port = cfg.rpc_port,
        user = cfg.rpc_user,
        password = cfg.rpc_password,
        min_conf = cfg.min_conf,
        threshold_zat = cfg.threshold_zat,
        health_port = cfg.rpc_port + 1,
    );
    std::fs::write(datadir.join("tparty.toml"), toml)?;
    Ok(())
}
