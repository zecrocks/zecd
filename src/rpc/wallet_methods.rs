...
pub(crate) fn getbalances(state: &AppState, wallet: Option<&str>) -> Result<Value, RpcError> {
    ...
    if let Some(h) = handle.status().fully_scanned {
        if let Ok(Some((hash, _))) = read::block_info_at(&handle.engine_dir, h) {
...
pub(crate) fn listsinceblock(
    ...
    let since_height = match req
        ...
        .filter(|s| !s.is_empty())
    {
        Some(hash) => match read::block_height_by_hash(&handle.engine_dir, hash)? {
    ...
    let lastblock = st
        ...
        .and_then(|h| read::block_info_at(&handle.engine_dir, h).ok().flatten())
        .map(|(hash, _)| hash)
        .or_else(|| {
            read::first_scanned_block(&handle.engine_dir)
                .ok()
                .flatten()
                .map(|(_, hash)| hash)
        })
