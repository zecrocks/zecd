...
fn wallet_ready(st: &SyncStatus, cfg: &HealthConfig) -> bool {
    ...
    match cfg.readiness {
        ...
        // completeness lags, and `getwalletinfo.enhanced_through` bounds that precisely. The
        ...
        // opt-out for deployments that clamp history consumption to `enhanced_through`.
...
mod tests {
    ...
    fn scanned_mode_requires_the_height_gap_but_ignores_the_enhancement_backlog() {
        ...
        // The reported shape: block scan at the tip, but a send just re-emitted the
        ...
        // only history completeness lags, and `enhanced_through` bounds that for consumers.
