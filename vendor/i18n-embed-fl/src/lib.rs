    ...
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        if !input.is_empty() {
            ...
            if args.is_empty() {
                ...
                // Sort the arguments so the macro output is deterministic: HashMap
                // iteration order is randomly seeded per process, which made builds
                // non-reproducible (kellpossible/cargo-i18n#151).
